...
spur_gear = SpurGear(module=0.828 / 2, teeth_number=20, width=5.0, bore_d=2.5, pressure_angle=ang+4)

right_spur_gear = SpurGear(module=0.8 / 2, teeth_number=20, width=5.0, bore_d=2.5, pressure_angle=ang)

# Build this gear using the gear function from cq.Workplane
# wp = cq.Workplane('XY').gear(spur_gear)

straight_ring = RingGear(module=0.8 / 2, teeth_number=20, width=5.0, rim_width=2.5, pressure_angle=ang, clearance=0.24, backlash=-1.8)
...
wp5 = cq.Workplane('XY').circle(7.0 / 2).extrude(5)

wp6 = cq.Workplane('XY').circle(8.8 / 2).extrude(5)
